<!DOCTYPE html>
<html lang="es">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
  <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/css/bootstrap.min.css">
  <link rel="stylesheet" href="css/style.css" />
  <title>Especialidades</title>
</head>
<body>

<div class="container mt-5">
  <h1 class="text-center card-title">Especialidades</h1>

  <div class="row mt-4">
    <div class="col-md-6">
      <div class="card mb-4 d-flex flex-column" style="max-height: 100%;">
        <div class="card-body">
          <h5 class="card-title">Medicina General</h5>
          <p class="card-text"> 
            La medicina general brinda atención médica integral a pacientes de todas las edades, abordando diagnóstico, tratamiento y prevención.
          </p>
          <a href="Medicina_general.html" class="btn btn-pers">Mostrar mas</a>
        </div>
      </div>
      <div class="card mb-4 d-flex flex-column" style="max-height: 100%;">
        <div class="card-body">
          <h5 class="card-title">Dermatología</h5>
          <p class="card-text">Diagnóstico y tratamiento de condiciones dermatológicas, como acné, eczema y cáncer de piel.</p>
          <a href="Dermatologia.html" class="btn btn-pers">Mostrar mas</a>
        </div>
      </div>
      <div class="card mb-4 d-flex flex-column" style="max-height: 100%;">
        <div class="card-body">
          <h5 class="card-title">Gastroenterología</h5>
          <p class="card-text">Diagnóstico y tratamiento de enfermedades gastrointestinales, como la enfermedad inflamatoria intestinal y la enfermedad del hígado.</p>
          <a href="Gastroenterología.html" class="btn btn-pers">Mostrar mas</a>
        </div>
      </div>

      <!-- Repite este bloque para las especialidades en la primera columna -->

    </div>

    <div class="col-md-6">
      <div class="card mb-4 d-flex flex-column" style="max-height: 100%;">
        <div class="card-body">
          <h5 class="card-title">Ginecología</h5>
          <p class="card-text">Manejo de la salud ginecológica, incluyendo embarazo, parto, y problemas del sistema reproductivo.</p>
          <a href="Ginecología.html" class="btn btn-pers">Mostrar mas</a>
        </div>
      </div>
      <div class="card mb-4 d-flex flex-column" style="max-height: 100%;">
        <div class="card-body">
          <h5 class="card-title">Odontología</h5>
          <p class="card-text">Trata problemas relacionados con la salud bucal y dental. Incluye limpiezas, obturaciones, extracciones y otros procedimientos dentales.</p>
          <a href="Odontología.html" class="btn btn-pers">Mostrar mas</a>
        </div>
      </div>
      <div class="card mb-4 d-flex flex-column" style="max-height: 100%;">
        <div class="card-body">
          <h5 class="card-title">Oftalmología </h5>
          <p class="card-text">Diagnóstico y tratamiento de condiciones oculares, como cataratas, glaucoma y problemas de la retina.</p>
          <a href="Oftalmología.html" class="btn btn-pers">Mostrar mas</a>
        </div>
      </div>

      <!-- Repite este bloque para las especialidades en la segunda columna -->

    </div>
  </div>

  <!-- Puedes seguir añadiendo más columnas según sea necesario -->

</div>


</body>
</html>
